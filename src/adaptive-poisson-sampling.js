...
export default function poisson(
  ...
  const point_tree = new Quadtree(9, ["[0]", "[1]", "[0]", "[1]"]); // accept [x, y] points
  ...
  const seed_point = toGrid([rng() * width, rng() * height]);
  ...
  while (active_points.length) {
    const current_point = active_points.splice(
      Math.floor(rng() * active_points.length),
      1
    )[0];
    ...
    while (attempts < max_attempts) {
      ...
      const radius =
        location_density +
        Math.sqrt(rng()) * location_density * range_reduction;
...
function inBox(bbox, point) {
  return (
    point[0] > 0 && point[0] < bbox[0] && point[1] > 0 && point[1] < bbox[1]
  );
}
